...
#### 결정
- **수동 저장 방식** 채택 (저장 버튼 클릭)
- Phase 1에서는 자동저장 기능 제외

#### 배경
초기 설계에서는 노션 스타일 자동저장을 고려했으나, Phase 1 간소화를 위해 제외.

#### 이유
- **구현 복잡도 감소**: debounce 로직, 변경 감지 등 제거
- **명확한 저장 시점**: 사용자가 명시적으로 저장 버튼 클릭
- **빠른 MVP 완성**: 핵심 기능에 집중
- **Phase 2 확장 가능**: 나중에 자동저장 추가 가능

#### 구현 방향
- 에디터 상단에 "저장" 버튼 배치
- 변경사항 있을 때 버튼 활성화 (dirty state 관리)
- 저장 성공/실패 토스트 메시지 표시
...
#### 결정
- **2단계 구조** 채택 (최상위 카테고리 + 서브카테고리)
- Phase 1에서는 Seed 스크립트로만 관리, UI 미제공

#### 배경
초기 설계에서는 Root/Trash 시스템과 3단계 계층 구조를 고려했으나, Phase 1 간소화를 위해 제외.

#### 이유
- **구현 복잡도 감소**: 재귀 쿼리, depth 검증 로직 간소화
- **충분한 구조**: 최상위 3개 + 서브카테고리로 블로그 운영 가능
- **관리 부담 감소**: Phase 1에서는 카테고리 변경 불필요
- **Phase 2 확장 가능**: 카테고리 관리 UI는 나중에 추가
...
#### 구현 방향
- Seed 스크립트로 초기 카테고리 생성
- Foreign Key Restrict로 하위 카테고리/글 있으면 삭제 불가
- 물리적 삭제만 지원 (Trash 시스템 제외)
...
4. **저장 방식** (Phase 1 간소화)
   - 이전: 자동저장 (debounce)
   - 변경: 수동 저장 (저장 버튼) → 자동저장은 Phase 2

5. **카테고리 구조** (Phase 1 간소화)
   - 이전: Root/Trash 시스템, 3단계 계층, 관리 UI
   - 변경: 2단계 구조, Seed 관리, UI는 Phase 2
